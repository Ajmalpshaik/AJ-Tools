 ...
 *
 * Input         : Document, a target View, selected category ids, and FilterRules built by FilterRuleBuilder.
 * Output        : The distinct ElementIds (instances only) currently matching, in that view.
 *
 * Notes         :
 * - No ParameterFilterElement is created and no view filter is touched — this only runs the same
 *   rule logic Filter Pro uses to build a live FilteredElementCollector query.
...
{
    /// <summary>
    /// Runs category + parameter-rule matching live against the active view (no saved filter).
    /// </summary>
