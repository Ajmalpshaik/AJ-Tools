#region Metadata
/*
 * Tool Name     : Purge Unused Filters
 * File Name     : CmdPurgeUnusedFilters.cs
 * Purpose       : Entry command that previews view/selection filters not applied on any view or view
 *                 template and deletes the ones the user selects. Delegates to the shared
 ...
 *
 * Input         : Full Project - unused filters; the user selects which to delete in the window.
 * Output        : Selected filters deleted; final report from the window.
 *
 * Notes         :
...
namespace AJTools.Commands
{
    ...
    public class CmdPurgeUnusedFilters : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            return UnusedElementPurgeCommandRunner.Execute(
                commandData,
                ref message,
                UnusedElementPurgeMode.Filters);
        }
    }
}
