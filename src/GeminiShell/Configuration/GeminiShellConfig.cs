...
namespace AJTools.GeminiShell.Configuration
{
    public class GeminiShellConfig
    {
        ...
        public static GeminiShellConfig Load()
        {
            lock (_fileLock)
            {
                if (File.Exists(ConfigPath))
                {
                    try
                    {
                        ...
                        return JsonConvert.DeserializeObject<GeminiShellConfig>(json) ?? new GeminiShellConfig();
                    }
                    catch (Exception ex)
                    {
                        LogError("Failed to load config", ex);
                        return new GeminiShellConfig();
                    }
                }
                return new GeminiShellConfig();
            }
