...
using (var t = new Transaction(Document, "AJ Tools - Split 200mm After Takeoff"))
{
    t.Start();
    foreach (var duct in mainDucts) {
        ...
        // Group takeoffs that are "very near or opposite side" (within 2.0 feet)
        ...
        foreach (var cluster in clusters) {
            ...
            // Safety: if the 200mm offset hits the next cluster, pull it back slightly
            ...
            // Only add if there's enough physical space on the duct
            if (splitDist < ductLength - 0.2) {
                breakDistances.Add(splitDist);
            }
        }
        
        // Order descending so breaking doesn't mess up upstream distance tracking
        ...
        foreach (var dist in uniqueBreaks) {
            if (dist > 0.2 && dist < ductLength - 0.2) {
                ...
                try {
                    ...
                    // Ignore failures where geometry is too tight
                }
            }
        }
    }
