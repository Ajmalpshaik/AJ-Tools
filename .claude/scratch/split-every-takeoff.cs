...
using (var t = new Transaction(Document, "AJ Tools - Split Every Takeoff"))
{
    t.Start();
    foreach (var duct in mainDucts) {
        ...
        // Group ONLY takeoffs that are exactly at the same spot (e.g. cross takeoffs)
        ...
        // Split halfway between EVERY takeoff!
        for (int i = 0; i < clusters.Count - 1; i++) {
            var upstreamMax = clusters[i].Max();
            var downstreamMin = clusters[i+1].Min();
            var midDist = (upstreamMax + downstreamMin) / 2.0;
            // Ensure there's a little bit of room for the union
            if (downstreamMin - upstreamMax > 0.5) breakDistances.Add(midDist);
        }
        
        var firstMin = clusters.First().Min();
        var lastMax = clusters.Last().Max();
        
        if (firstMin > 3.0) breakDistances.Add(firstMin - 1.5);
        if (ductLength - lastMax > 3.0) breakDistances.Add(lastMax + 1.5);
