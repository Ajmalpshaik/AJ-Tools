...
using (var t = new Transaction(Document, "AJ Tools - Split Downstream"))
{
    t.Start();
    foreach (var duct in mainDucts) {
        ...
        foreach (var place in connectingPlaces) {
            var splitDist = place + 1.7; // Exactly 1.7 feet downstream
            
            // Just in case the next takeoff is extremely close, avoid colliding with it
            var nextPlace = connectingPlaces.FirstOrDefault(p => p > place);
            if (nextPlace > 0 && splitDist > nextPlace - 0.5) {
                splitDist = (place + nextPlace) / 2.0;
            }
            
            if (splitDist < ductLength - 0.5) {
        ...
        foreach (var dist in uniqueBreaks) {
            if (dist > 0.5 && dist < ductLength - 0.5) {
                var breakPt = p0 + dir * dist;
                try {
