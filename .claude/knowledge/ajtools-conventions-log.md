...
- Goal was to make the AJ AI Bridge competitive with a third-party product (Nonica's Revit MCP connector)
...
- **Re-architected `.claude/scripts/` into filter+action fragments (Ajmal's idea, same day, confirmed
  ...
  select). Borrowed one idea from a quick look at github.com/ProfRino/Nonica-Revit-MCP-Skill (a
...
- **Stability/upgrade pass on `.claude/scripts/`, same day**, after Ajmal asked to check
  github.com/mcp-servers-for-revit/revit-mcp-commandset for anything worth taking. Different
...
- **Second research pass, same day** — Ajmal pointed at 5 more repos (mcp-servers-for-revit's monorepo/
  server/plugin, shuotao/REVIT_MCP_study, DTDucas/RevitMCPSDK) plus a GitHub search sweep. Again no code
  ...
  - Created **[`.claude/tools/verify-knowledge-consistency.ps1`](../tools/verify-knowledge-consistency.ps1)**
    after seeing shuotao/REVIT_MCP_study run its own QA/QC pass over its docs — checks every SKILL.md
  ...
  - Confirmed AutoDebugger's `run_csharp` already handles the Revit-main-thread/ExternalEvent
    thread-safety concern that `revit-mcp-plugin`'s `ExternalEventManager` exists to solve — reviewed
